...
#         https://blog.shikoan.com/pillow-imageops/#11_Mirror
        if np.random.randint(2):
            img = ImageOps.flip(img)
            anno_class_img = ImageOps.flip(anno_class_img)
        return img, anno_class_img
...
class Resize(object):
    ...
    def __call__(self, img, anno_class_img):
        ...
        img = img.resize((self.input_size[0], self.input_size[1]),
                         Image.BICUBIC)
        anno_class_img = anno_class_img.resize(
            (self.input_size[0], self.input_size[1]), Image.BICUBIC)
...
class Normalize_Tensor(object):
    ...
    def __call__(self, img, anno_class_img):
        ...
        # 逆変換 : 元画像 = transforms.functional.to_pil_image(img)
        img = transforms.functional.to_tensor(img)
        anno_class_img = transforms.functional.to_tensor(anno_class_img)
...
class Compose_angle(object):
    ...
    def __call__(self, img, anno_color_class_img, anno_mask_class_img):
        for t in self.transforms:
            img, anno_color_class_img, anno_mask_class_img = t(img, anno_color_class_img, anno_mask_class_img)
        return img, anno_color_class_img, anno_mask_class_img
    
    
class Resize_angle(object):
    """引数input_sizeに大きさを変形するクラス"""
    ...
    def __call__(self, img, anno_color_class_img, anno_mask_class_img):
        ...
        img = img.resize((self.input_size[0], self.input_size[1]),
                         Image.BICUBIC)
        anno_color_class_img = anno_color_class_img.resize(
            (self.input_size[0], self.input_size[1]), Image.NEAREST)
        anno_mask_class_img = anno_mask_class_img.resize(
            (self.input_size[0], self.input_size[1]), Image.BILINEAR)

        return img, anno_color_class_img, anno_mask_class_img
    
    
class Normalize_Tensor_angle(object):
    def __init__(self, color_mean, color_std):
        self.color_mean = color_mean
        self.color_std = color_std
    ...
    def __call__(self, img, anno_color_class_img, anno_mask_class_img):
        ...
        # 逆変換 : 元画像 = transforms.functional.to_pil_image(img)
        img = transforms.functional.to_tensor(img)
        anno_color_class_img = transforms.functional.to_tensor(anno_color_class_img)
        anno_mask_class_img = transforms.functional.to_tensor(anno_mask_class_img)
